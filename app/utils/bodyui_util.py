...
class BodyUIUtil:
    ...
    def on_join_group(group: Group, account: Account, label: CTkLabel, btn_cancel_state: CTkButton):
        try:
            if group.account_list.__len__() < BodyUIUtil.max_member and account.group_list == []:
                group.account_list.append(account)
                BodyUIUtil.group_dao.update(group)
                print(f'Join group: {group.name}')
                label.configure(text=f'Member: {group.account_list.__len__()} / {BodyUIUtil.max_member}')
